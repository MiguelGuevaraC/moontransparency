...
class UpdateActivityRequest extends UpdateRequest
{
    ...
    {
        $activityId = $this->route('id'); // Asegúrate que el parámetro en la ruta sea 'activity'
        ...
        return [
            ...
            'total_amount' => [
                ...
                function ($attribute, $value, $fail) use ($activity) {
                    if ($activity && $value < $activity->collected_amount) {
                        $fail("El monto total no puede ser menor al monto recaudado ({$activity->collected_amount}).");
                    }
                },
            ],
            'status'       => ['nullable', 'string', 'in:PENDIENTE,EN EJECUCION,FINALIZADO'],
        ];
    }
